...
return new class extends Migration
{
    ...
    {
        // On vérifie si la table n'existe pas déjà avant de tenter de la créer
        if (!Schema::hasTable('comptes')) {
            Schema::create('comptes', function (Blueprint $table) {
                ...
                // Clés étrangères
                $table->foreignId('client_id')->constrained('clients')->cascadeOnDelete()->comment('Référence au client propriétaire');
                ...
                // Les index (Note: foreignId crée déjà des index, mais on peut les expliciter ou indexer d'autres champs)
                $table->index('statut');
                $table->index('devise');
                $table->index('numero_compte'); // Déjà indexé car unique, mais utile pour la clarté
            });
        }
    }
