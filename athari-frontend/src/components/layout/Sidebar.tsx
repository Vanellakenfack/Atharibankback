  ...
  return (
    ...
    relative z-10'>
      ...
      <nav className='flex-1 p-4 space-y-2 overflow-y-auto'>
        {menuItems.map((item) => {
          const Icon = item.icon // ✅ Récupérer le composant icône
                ...
                <div className='ml-6 mt-2 space-y-1'>
                  {item.submenu.map((subItem) => (
                    <a
                      key={subItem.id}
                      href={subItem.link}
                      className='block p-2 pl-4 text-sm text-slate-600 dark:text-slate-400 
                        hover:text-blue-600 dark:hover:text-blue-400 
                        hover:bg-slate-50 dark:hover:bg-slate-800 
                        rounded-lg transition-colors border-l-2 border-slate-200 
